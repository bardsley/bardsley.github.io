...
function EnableAutoLoadMore(e, t) {
    $(window).on("scroll", function () {
        ("undefined" == typeof $(e).first().attr("data-loadmore") || 0 == $(e).first().attr("data-loadmore")) && $(e).first().attr("data-loadmore", 0), $(e + ":visible").length > 0 && $(window).height() + $(document).scrollTop() > $(e).first().offset().top - 80 && $(t).first().children().length > $(e).first().attr("data-loadmore") && ($(e).first().attr("data-loadmore", $(t).first().children().length), $(e).trigger("click"))
    })
}
...
$(function () {
    ...
    if ($("#union-logo a").attr("href", "https://www.salfordstudents.com/"), $("#union-logo, .system-menu a.uc-sign-link, .profile-setting a.profile-name").addClass("transition-enabled"), "/" == window.location.pathname && (LoadEventsListing(), /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || $(document).ajaxComplete(UpdateHomeImages)), $("#uc-left-navigation a.selected").parent().children("ul").css("display", "block"), $(".nav-level-one a").on("click", function (e) { e.preventDefault(), $("#uc-left-navigation").toggleClass("visible") }), window.location.pathname.startsWith("/users/sign_in") && ($("form :input").each(function (e, t) {
        var n = $(t).attr("id"),
            a = $(t).parents("form").find("label[for=" + n + "]");
        n && 1 == a.length && $(t).attr("placeholder", $(a).html().trim())
    }), $(".uc-back-button-hyper .back-link-button").html("Back to login options")), window.location.pathname.startsWith("/users/authenticate_email") && $(".existing-user .click-here-text").length) {
    ...
    if (window.location.pathname.startsWith("/events") && window.location.pathname.length <= 8, window.location.pathname.startsWith("/events") && window.location.pathname.length > 8 && $(".galleryIcon img").attr("src", $(".galleryIcon img").attr("src").replace("/medium/", "/original/")), window.location.pathname.startsWith("/groups") && window.location.pathname.includes("/events/") && $(".threecol .title h1").html($("#uc-events-details-page .contentBoxes").first().find("h2").html()), window.location.pathname.startsWith("/groups") && window.location.pathname.length <= 8 && (EnableAutoLoadMore("#uc-more-group-search", ".category-box-wrapper"), $(".club-navigation ul li").on("click", function () { $("#uc-more-group-search").first().attr("data-loadmore", 0) }), $(".group-types-wrapper ul li").on("click", function () { $("#uc-more-group-search").first().attr("data-loadmore", 0) })), window.location.pathname.endsWith("/join") || window.location.pathname.endsWith("join/")) {
    ...
    if (window.location.pathname.startsWith("/users/customer_registration") && ($(".uc-signed_inuser_msg").html("<h3>Have a University login / already registered? Sign in below:</h3>"), $(".uc-unsigned_inuser_msg").html("<h3>Checkout as a guest:</h3>")), (window.location.pathname.endsWith("/articles") || window.location.pathname.endsWith("/articles/")) && ($(".uc-articles-heading h1").html("Blog"), document.title = "Blog @ University of Salford Students' Union", $(".uc-articles-right-panel-banner").remove(), $(".articles-full-banner-advert").remove(), $(".uc-search-articles-filters h3").html("Filter by Category"), $(".uc-filter-block h3").die("click"), EnableAutoLoadMore("#load_more_article", ".uc-articles-listing")), window.location.pathname.includes("/articles/") && !window.location.pathname.endsWith("/articles") && !window.location.pathname.endsWith("/articles/")) {
    ...
    } window.location.pathname.startsWith("/thestudentvoice") && window.location.pathname.length <= 17 && ($(".uc-heading-title span").html("Big Ideas"), document.title = "Big Ideas @ University of Salford Student's Union", $(".uc-sv-post-idea-petition a").html("Post An Idea"), EnableAutoLoadMore("#load_more_sv", ".uc-sv-left-panel-wrapper"), $("#uc-sv-navigation li a").on("click", function () { $("#load_more_sv").first().attr("data-loadmore", 0) })), $(".image-slideshow").each(function (e) {
        var t = this,
            n = $(t).attr("data-speed");
        n = "undefined" == typeof n || 0 == n ? 5 : parseInt(n), $(t).children("img:first").addClass("active-image"), setInterval(function () { CycleImage(t) }, 1e3 * n)
    })
